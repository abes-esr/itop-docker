...
{ echo "# Synchro itop data"; \
	...
	echo "*/5 * * * * www-data /usr/local/bin/php /var/www/itop/webservices/cron.php --auth_user=${ITOP_ADMIN} --auth_pwd='${ITOP_ADMIN_PASS}' --debug=1 --size_min=20 --time_limit=40 >> /var/log/itop-cron.log 2>&1"; \
	} | crontab -
# start cron
echo "Starting cron..."
service cron start
